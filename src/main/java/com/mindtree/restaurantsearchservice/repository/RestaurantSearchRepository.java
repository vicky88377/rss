...
public interface RestaurantSearchRepository extends ElasticsearchRepository<RestaurantModel, String> {
	...
	public Page<RestaurantModel> findByLonLatAndName(String restaurantName, float distance, double latitude, double longitude,
			Pageable page);
	
	// Getting restaurant details by location, distance & cuisine_type(mandatory), rating and budget(optional) parameters
	...
	public Page<RestaurantModel> findByLonLatRatingBudget(String cuisineType, float rating, float minimumOrderPrice, float distance, double latitude, double longitude,
			Pageable page);
