...
public class RestaurantSearchServiceImpl implements RestaurantSearchServiceInterface {
	...
	/*
	 * @Override public Page<RestaurantModel> getRestaurantByArea(String location,
	 * int pageNo) { if (logger.isDebugEnabled()) {
	 * logger.debug("params data: location: " + location + " pageNo:" + pageNo); }
	 * Pageable pageable = PageRequest.of(pageNo, pageSize); float rating = 0,
	 * budget = 0; Page<RestaurantModel> data =
	 * restaurantRepo.findByAreaRatingBudgetDAO(location, rating, budget, pageable);
	 * if (data != null && logger.isDebugEnabled()) {
	 * logger.debug("response data: data: " + data.getContent()); } return data; }
	 */
	...
	public Page<RestaurantModel> getRestaurantByLocationAndFilterParam(double latitude, double longitude,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("param data: latitude: " + latitude + " longitude: " + longitude + " cuisine: " + cuisine
					+ " budget: " + budget + " rating: " + rating + " restauratnName: " + name + " pageNo: " + pageNo);
		}
		Page<RestaurantModel> data = null;
		if (name != null && !name.isEmpty()) {
	...
	public Page<RestaurantModel> getRestaurantByLocation(double latitude, double longitude, float distance,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("param data: latitude: " + latitude + " longitude: " + longitude + " distance: " + distance
					+ " pageNo: " + pageNo);
		}
	...
	/**
	 * {@link:https://stackoverflow.com/questions/3694380/calculating-distance-between-two-points-using-latitude-longitude-what-am-i-doi} Calculate
	 * distance between two points in latitude and longitude taking into account
	 * height difference. If you are not interested in height difference pass 0.0.
